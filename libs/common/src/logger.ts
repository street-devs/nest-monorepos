...
export const APP_LOG_LEVEL: pino.LevelWithSilent = envUnionOptional(
  ['fatal', 'error', 'warn', 'info', 'debug', 'trace', 'silent'],
  'info',
  'APP_LOG_LEVEL'
)

const USE_PRETTY_LOG = envBooleanOptional(false, 'PRETTY_LOGGER')

export const logger = pino({
  messageKey: 'message',
  timestamp: () => `,"@timestamp":"${new Date().toISOString()}"`,
  redact: {
    paths: ['req.headers.authorization', 'req.headers.cookie', 'req.query'],
  },
  formatters: {
...
export function createLogger(prefix: string, bindings?: any, level?: string) {
  return logger.child(
    { logger: getDefaultContext(), ...bindings },
    {
      msgPrefix: prefix,
      level: level ?? APP_LOG_LEVEL,
    }
  )
}
